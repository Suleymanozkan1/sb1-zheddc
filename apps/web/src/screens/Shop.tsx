...
import { Button, Panel, RarityBadge, Spinner, cx } from "@cryptoarena/ui";
...
export function Shop() {
  ...
  return (
    <Page title="Shop" subtitle="Prices are managed server-side. Purchases buy in-game items, cosmetics and utility — never financial returns.">
          ...
          {products.map((p) => (
            <Panel key={p.id} className={cx("flex flex-col", p.metadata.highlight && "border-fuchsia-400/50")}>
              <div className="mb-2 flex items-start justify-between gap-2">
                <div>
                  <p className="text-[10px] tracking-widest text-slate-400 uppercase">{LABEL[p.category]}</p>
              ...
              <Button variant={p.currency === "CRYPTO" ? "primary" : "secondary"} disabled={p.owned} loading={busy === p.sku} onClick={() => void buy(p)}>
                {p.owned ? "Owned" : `${CURRENCY_ICON[p.currency]} ${price(p.price, p.currency, me?.balances)}`}
              </Button>
              {p.currency === "CRYPTO" && <p className="mt-2 text-[10px] text-slate-500">Paid from deposited balance first, then reward balance.</p>}
            </Panel>
          ))}
        </div>
      )}
