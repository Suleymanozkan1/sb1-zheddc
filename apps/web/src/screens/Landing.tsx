...
export function Landing() {
  ...
  return (
    <div className="grid-bg relative flex min-h-full flex-col items-center justify-center overflow-hidden px-4 py-16 text-center">
      ...
      <p className="mb-4 text-xs font-semibold tracking-[0.4em] text-cyan-300/80 uppercase">{t("Real-time multiplayer · Solana devnet")}</p>
      ...
      <p className="mt-6 max-w-2xl text-slate-300 md:text-lg">
        {t("Pick a hero, drop into a 10,000×10,000 neon battlefield, farm resources, slay creatures and outplay rivals. Earn gear, climb seasonal leaderboards and win performance-based rewards.")}
      </p>
      <Button variant="primary" size="xl" className="mt-10 font-display" onClick={() => (DEMO_ONLY ? void startDemo() : setOpen(true))}>
        {DEMO_ONLY ? t("PLAY DEMO") : t("PLAY NOW")}
      </Button>
      {DEMO_ONLY && (
        <p className="mt-3 max-w-md text-xs text-slate-400">
          {t("Offline demo: the arena runs in your browser against bots. Progress is saved on this device; wallets and crypto rewards are disabled.")}
        </p>
      )}
      <div className="mt-12 grid max-w-4xl gap-4 text-left sm:grid-cols-3">
        {[
          ...
          <div key={title} className="glass p-4">
            <h3 className="font-display text-sm font-bold text-cyan-200">{t(title!)}</h3>
            <p className="mt-1 text-sm text-slate-400">{t(d!)}</p>
          </div>
        ))}
      </div>
      <p className="mt-10 max-w-2xl text-xs text-slate-500">
        {t("CryptoArena is a game, not an investment. Purchases buy in-game items and utility only. Rewards depend on gameplay performance, are paid from finite reward budgets and are never guaranteed. Availability of crypto features may be restricted in some regions.")}
      </p>

      <Modal open={open} onClose={() => setOpen(false)} title={t("Enter the arena")}>
        <div className="flex flex-col gap-3">
          <Button variant="primary" size="lg" loading={busy} onClick={() => void signIn()}>
            {t("Connect Wallet (Phantom · Solflare · Backpack)")}
          </Button>
          <Button
            size="lg"
            loading={guestBusy}
          ...
          <Button size="lg" variant="ghost" onClick={() => void startDemo()}>
            {t("Try the offline demo")}
          </Button>
          <p className="text-xs text-slate-400">
            {t("Guests keep their progress on this device and can link a wallet later to unlock deposits, crypto rewards and withdrawals. The offline demo runs entirely in your browser against bots, without an account.")}
          </p>
        </div>
      </Modal>
    </div>
