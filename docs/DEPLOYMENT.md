...
5. **Game server**: WebSocket-capable load balancer with sticky sessions; one process per core; expose `wss://`. The one-live-character-per-account rule holds across processes through the `GameSeat` lease table (45 s lease, renewed every 15 s).
